"""[summary]
"""
import json
from cybercastor.classes.RiverscapesAPI import RiverscapesAPI
from rscommons import Logger
import inquirer

huc_groups = {
    "Big Hole, Southwest Montana": [
        "1002000101",
        "1002000102",
        "1002000103",
        "1002000104",
        "1002000105",
        "1002000106",
        "1002000107",
        "1002000108",
        "1002000109",
        "1002000110",
        "1002000111",
        "1002000112",
        "1002000113",
        "1002000114",
        "1002000201",
        "1002000202",
        "1002000203",
        "1002000204",
        "1002000205",
        "1002000206",
        "1002000207",
        "1002000301",
        "1002000302",
        "1002000303",
        "1002000304",
        "1002000305",
        "1002000401",
        "1002000402",
        "1002000403",
        "1002000404",
        "1002000405",
        "1002000406",
        "1002000407",
        "1002000408",
        "1002000409",
        "1002000410",
        "1002000411",
        "1002000412",
        "1002000413",
        "1002000701",
        "1002000702",
        "1002000703",
        "1002000704",
        "1002000705",
        "1002000706",
        "1002000707",
        "1002000708",
        "1002000709",
        "1002000710",
        "1002000711",
        "1002000712",
        "1002000713",
        "1002000714",
        "1002000715"],
    "The Sagebrush Anchor, North Central Montana": [
        "1005001201",
        "1005001202",
        "1005001203",
        "1005001204",
        "1005001205",
        "1005001206",
        "1005001207",
        "1005001208",
        "1005001209",
        "1005001210",
        "1005001401",
        "1005001402",
        "1005001403",
        "1005001404",
        "1005001405",
        "1005001406",
        "1005001407",
        "1005001501",
        "1005001502",
        "1005001503",
        "1005001504",
        "1005001505",
        "1005001506",
        "1005001507",
        "1005001508",
        "1005001509"],
    "North Park": [
        "1018000101",
        "1018000102",
        "1018000103",
        "1018000104",
        "1018000105",
        "1018000106",
        "1018000107"],
    "San Luis Valley": [
        "1301000201",
        "1301000202",
        "1301000203",
        "1301000204",
        "1301000205",
        "1301000206",
        "1301000207",
        "1301000208",
        "1301000209",
        "1301000210",
        "1301000211",
        "1301000301",
        "1301000302",
        "1301000303",
        "1301000304",
        "1301000305",
        "1301000306",
        "1301000307"],
    "Lower Pecos": [
        "1306000301",
        "1306000302",
        "1306000303",
        "1306000304",
        "1306000305",
        "1306000306",
        "1306000307",
        "1306000308",
        "1306000309",
        "1306000310",
        "1306000311",
        "1306000312",
        "1306000313",
        "1306000314",
        "1306000315",
        "1306000316",
        "1306000317",
        "1306000318",
        "1306000701",
        "1306000702",
        "1306000703",
        "1306000704",
        "1306000705",
        "1306000706",
        "1306000707",
        "1306000708",
        "1306000709",
        "1306000710",
        "1306000711",
        "1306000712",
        "1306000713",
        "1306000714",
        "1306001101",
        "1306001102",
        "1306001103",
        "1306001104",
        "1306001105",
        "1306001106",
        "1306001107",
        "1306001108",
        "1306001109",
        "1306001110",
        "1306001111",
        "1306001112",
        "1306001113",
        "1306001114",
        "1306001115",
        "1306001116",
        "1306001117"],
    "La Barge": [
        "1404010101",
        "1404010102",
        "1404010103",
        "1404010104",
        "1404010105",
        "1404010106",
        "1404010107",
        "1404010108",
        "1404010109",
        "1404010110",
        "1404010111",
        "1404010112",
        "1404010113",
        "1404010201",
        "1404010202",
        "1404010203",
        "1404010204",
        "1404010205",
        "1404010206"],
    "Muddy Creek": [
        "1405000401",
        "1405000402",
        "1405000403"],
    "Yanawant": [
        "1501000201",
        "1501000202",
        "1501000203",
        "1501000204",
        "1501000206",
        "1501000207",
        "1501000208",
        "1501000210",
        "1501000301",
        "1501000302",
        "1501000303",
        "1501000304",
        "1501000305",
        "1501000306",
        "1501000307",
        "1501000308",
        "1501000309",
        "1501000310",
        "1501000601",
        "1501000602",
        "1501000603",
        "1501000604",
        "1501000605",
        "1501000606"],
    "Sky Islands": [
        "1504000502",
        "1504000503",
        "1504000504",
        "1504000505",
        "1504000506",
        "1504000507",
        "1504000508",
        "1504000509",
        "1505020203",
        "1505020205",
        "1505020206",
        "1505020207",
        "1505020208",
        "1505020209",
        "1505020210",
        "1505020211",
        "1505020301",
        "1505020302",
        "1505020303",
        "1505020304",
        "1505020305",
        "1505020306",
        "1505020307",
        "1505020308",
        "1505020309",
        "1505030201",
        "1505030202",
        "1505030203"],
    "Upper Bear River": [
        "1601010101",
        "1601010102",
        "1601010103",
        "1601010104",
        "1601010105",
        "1601010106",
        "1601010107",
        "1601010108",
        "1601010109"],
    "Color Country Converging": [
        "1501000801",
        "1501000802",
        "1501000803",
        "1501000804",
        "1501000805",
        "1501000806",
        "1501000807",
        "1501000808",
        "1501000809",
        "1603000601",
        "1603000602",
        "1603000603",
        "1603000604",
        "1603000605",
        "1603000606",
        "1603000607",
        "1603000608",
        "1603000609",
        "1603000610",
        "1603000611",
        "1603000612",
        "1603000613",
        "1603000614"],
    "Humboldt O'Neil Basin": [
        "1604010101",
        "1604010102",
        "1604010103",
        "1604010104",
        "1604010105",
        "1604010106",
        "1604010107",
        "1604010108",
        "1604010109",
        "1604010110",
        "1604010111",
        "1604010112",
        "1604010201",
        "1604010202",
        "1604010203",
        "1604010204",
        "1704021301",
        "1704021302",
        "1704021303",
        "1704021304",
        "1704021305",
        "1704021306",
        "1704021307",
        "1704021308",
        "1704021309"],
    "Montana Mountains": [
        "1604020101",
        "1604020102",
        "1604020103",
        "1604020104",
        "1604020105",
        "1604020106",
        "1604020107",
        "1604020108",
        "1604020109",
        "1604020110",
        "1604020111",
        "1604020112",
        "1604020113",
        "1604020201",
        "1604020203",
        "1604020204",
        "1604020205",
        "1604020206",
        "1604020207",
        "1604020208",
        "1604020209",
        "1604020210",
        "1604020211",
        "1604020212",
        "1604020213",
        "1604020214",
        "1604020215",
        "1604020216"],
    "Blackfoot-Clark Fork": [
        "1701020201",
        "1701020202",
        "1701020203",
        "1701020204",
        "1701020205",
        "1701020206",
        "1701020207",
        "1701020208",
        "1701020209",
        "1701020210",
        "1701020211",
        "1701020212",
        "1701020213",
        "1701020214",
        "1701020301",
        "1701020302",
        "1701020303",
        "1701020304",
        "1701020305",
        "1701020306",
        "1701020307",
        "1701020308",
        "1701020309",
        "1701020310",
        "1701020311",
        "1701020312",
        "1701020313"],
    "East Idaho Rivers and Plains": [
        "1704010401",
        "1704010402",
        "1704010403",
        "1704010404",
        "1704010405",
        "1704010406",
        "1704010407",
        "1704010408",
        "1704010409",
        "1704020301",
        "1704020302",
        "1704020303",
        "1704020304",
        "1704020305",
        "1704020401",
        "1704020402",
        "1704020403",
        "1704020404",
        "1704020405",
        "1704020406",
        "1704020407",
        "1704020408",
        "1704020501",
        "1704020502",
        "1704020503",
        "1704020504",
        "1704020701",
        "1704020702",
        "1704020703",
        "1704020704",
        "1704020705",
        "1704020706",
        "1704020801",
        "1704020802",
        "1704020803",
        "1704020804",
        "1704020805",
        "1704020806"],
    "Snake River Plain": [
        "1704021201",
        "1704021202",
        "1704021203",
        "1704021204",
        "1704021205",
        "1704021206",
        "1704021207",
        "1704021208",
        "1704021209",
        "1704021210",
        "1704021211",
        "1704021212",
        "1704021213",
        "1705010101",
        "1705010102",
        "1705010103",
        "1705010104",
        "1705010105",
        "1705010106",
        "1705010107",
        "1705010108",
        "1705010109",
        "1705010110",
        "1705010111",
        "1705010301",
        "1705010302",
        "1705010303",
        "1705010304",
        "1705010305",
        "1705010306",
        "1705010307",
        "1705010308",
        "1705010309",
        "1705010310",
        "1705010311",
        "1705011401",
        "1705011402",
        "1705011403",
        "1705011404",
        "1705011405",
        "1705011406",
        "1705011408",
        "1705011501",
        "1705011502"],
    "Upper Salmon River": [
        "1706020101",
        "1706020102",
        "1706020103",
        "1706020104",
        "1706020105",
        "1706020106",
        "1706020107",
        "1706020108",
        "1706020109",
        "1706020110",
        "1706020111",
        "1706020112",
        "1706020113",
        "1706020114",
        "1706020115",
        "1706020116",
        "1706020117",
        "1706020118",
        "1706020201",
        "1706020202",
        "1706020203",
        "1706020301",
        "1706020302",
        "1706020303",
        "1706020304",
        "1706020305",
        "1706020306",
        "1706020307",
        "1706020308",
        "1706020309",
        "1706020310",
        "1706020311",
        "1706020312",
        "1706020313",
        "1706020401",
        "1706020402",
        "1706020403",
        "1706020404",
        "1706020405",
        "1706020406",
        "1706020407",
        "1706020408"],
    "Southwest Oregon": [
        "1710030101",
        "1710030102",
        "1710030103",
        "1710030104",
        "1710030105",
        "1710030106",
        "1710030107",
        "1710030108",
        "1710030109",
        "1710030110",
        "1710030111",
        "1710030201",
        "1710030202",
        "1710030203",
        "1710030204",
        "1710030205",
        "1710030206",
        "1710030207",
        "1710030208",
        "1710030209",
        "1710030210",
        "1710030211",
        "1710030212",
        "1710030213",
        "1710030301",
        "1710030302",
        "1710030303",
        "1710030304",
        "1710030305",
        "1710030306",
        "1710030307",
        "1710030308",
        "1710030501",
        "1710030502",
        "1710030503",
        "1710030504",
        "1710030505",
        "1710030701",
        "1710030702",
        "1710030703",
        "1710030704",
        "1710030705",
        "1710030706",
        "1710030707",
        "1710030708",
        "1710030801",
        "1710030802",
        "1710030803",
        "1710030804",
        "1710030901",
        "1710030902",
        "1710030903",
        "1710030904",
        "1710030905",
        "1710030906",
        "1710031001",
        "1710031002",
        "1710031003",
        "1710031004",
        "1710031005",
        "1710031006",
        "1710031007",
        "1710031008"],
    "Southeast Oregon Sagebrush": [
        "1712000301",
        "1712000302",
        "1712000303",
        "1712000304",
        "1712000701",
        "1712000702",
        "1712000703",
        "1712000704",
        "1712000705",
        "1712000706",
        "1712000707",
        "1712000708",
        "1712000801",
        "1712000802",
        "1712000803",
        "1712000804",
        "1712000805",
        "1712000806",
        "1712000807",
        "1712000808",
        "1712000809",
        "1712000810",
        "1712000811",
        "1712000901",
        "1712000902",
        "1712000903",
        "1712000904",
        "1712000905",
        "1712000906",
        "1712000907",
        "1712000908",
        "1604020501",
        "1604020502",
        "1604020503",
        "1604020504",
        "1604020505",
        "1604020506"],
    "Cosumnes": [
        "1804001201",
        "1804001202",
        "1804001203",
        "1804001204",
        "1804001205",
        "1804001206",
        "1804001207",
        "1804001208",
        "1804001209",
        "1804001210",
        "1804001211",
        "1804001301",
        "1804001302",
        "1804001303",
        "1804001304",
        "1804001305",
        "1804001306",
        "1804001307",
        "1804001308"]
}


def add_tag(stage, tag: str):
    """ Find and add tag to projects on the server

    Args:
        stage (str): The server to run the script on
        tag (str): The tag to add to the projects
    """
    log = Logger('AddTag')
    log.title('Add tag to Projects from the server')

    riverscapes_api = RiverscapesAPI(stage=stage)
    search_query = riverscapes_api.load_query('searchProjects')
    # Only refresh the token if we need to
    if riverscapes_api.access_token is None:
        riverscapes_api.refresh_token()

    searchParams = {
        "meta": [{
            "key": "Runner",
            "value": "Cybercastor",
        }],
        "projectTypeId": 'anthro',
        "createdOn": {
            # "from": "2023-06-19",
            "to": "2023-06-19"
        }
    }

    changeable_projects = []
    offset = 0
    total = 0

    huc_list = sum([huc for huc in huc_groups.values()], [])

    # Create a timedelta object with a difference of 1 day
    while offset == 0 or offset < total:

        results = riverscapes_api.run_query(
            search_query, {"searchParams": searchParams, "limit": 500, "offset": offset})
        total = results['data']['searchProjects']['total']
        offset += 500

        projects = results['data']['searchProjects']['results']
        log.info(f"   Fetching projects {offset} to {offset + 500}")
        for search_result in projects:

            project = search_result['item']
            if next(d['value'] for d in project['meta'] if d['key'] == 'HUC') in huc_list:
                changeable_projects.append(project)

    # Now write all projects to a log file as json
    with open(f'add_tag_{stage}_{tag}', 'w') as f:
        f.write(json.dumps(changeable_projects))

    # Ask the user to confirm using inquirer
    log.info(f"Found {len(changeable_projects)} out of {total} projects to add tag")
    questions = [
        inquirer.Confirm('confirm1',
                         message=f"Are you sure you want to add the tag {tag} to all these projects?"),
    ]
    answers = inquirer.prompt(questions)
    if not answers['confirm1']:  # or not answers['confirm2']:
        log.info("Good choice. Aborting!")
        return

    # Now Change Owner of all projects
    mutation_script = riverscapes_api.load_mutation('updateProject')
    for project in changeable_projects:
        print(f"Add Tag to project: {project['name']} with id: {project['id']}")
        huc_id = next(d['value'] for d in project['meta'] if d['key'] == 'HUC')
        huc_group = next(k for k, v in huc_groups.items() if huc_id in v)
        if huc_group not in project['tags']:
            project['tags'].append(huc_group)
        if tag not in project['tags']:
            project['tags'].append(tag)
        riverscapes_api.run_query(mutation_script, {
            "projectId": project['id'],
            "project": {"tags": project['tags']}
        })

    # Shut down the API since we don;t need it anymore
    riverscapes_api.shutdown()

    log.info("Done!")


if __name__ == '__main__':
    add_tag('production', 'zzzz')
