...
dgo_fields = {
    ...
    'active_floodplain_prop': ogr.OFTReal,
    'active_channel_area': ogr.OFTReal,
    'active_channel_prop': ogr.OFTReal,
    'inactive_floodplain_area': ogr.OFTReal,
    'inactive_floodplain_prop': ogr.OFTReal,
    'floodplain_area': ogr.OFTReal,
    'floodplain_prop': ogr.OFTReal,
...
def download_gpkg(riverscapes_api: str, queries: dict, top_level_dir: str, project_guid: str, dataset_xml_id: str) -> str:
    ...
    if os.path.isfile(actual_path) is False:
        raise Exception(f'Unable to download file {local_path} from {download_file_meta["downloadUrl"]}')
...
def process_polygon_layer_attribute(polygon_layer: str, in_field_name: str, out_field_name: str, vbet_inter_gpkg: str, output_db: str, huc: str) -> str:
    ...
    for dgo_feature in dgo_layer:
        ...
        if len(intersection_areas) > 0:
            ...
            dgo_results.append((max_area, huc, dgo_level_path, dgo_seg_distance))

    # Update the DGO table with the results
    if len(dgo_results) > 0:
        with sqlite3.connect(output_db) as conn:
            curs = conn.cursor()
            curs.executemany(f'UPDATE vbet_dgos SET {out_field_name} = ? WHERE HUC10 = ? AND LevelPathI = ? AND seg_distance = ?', dgo_results)
            conn.commit()
...
def process_fcodes(vbet_input_gpkg: str, vbet_inter_gpkg: str, output_gpkg: str, huc: str) -> None:
    ...
    for dgo_feature in dgo_layer:
        ...
        if len(intersection_areas) > 0:
            ...
            dgo_results.append((max_length, huc, dgo_level_path, dgo_seg_distance))
    ...
    with GeopackageLayer(vbet_output_gpkg, layer_name='vbet_igos') as input_lyr:
        with GeopackageLayer(scrape_output_gpkg, layer_name='vbet_igos', write=True) as output_lyr:
            ...
            for in_feature, _counter, _progbar in input_lyr.iterate_features():
                field_values = {field: in_feature.GetField(field) for field in igo_fields if field != 'HUC10'}
    ...
    with sqlite3.connect(scrape_output_gpkg) as conn:
        ...
        with GeopackageLayer(vbet_intermediate_gpkg, layer_name='vbet_dgos') as input_lyr:
            for in_feature, _counter, _progbar in input_lyr.iterate_features():
                ...
                field_key_values = {field: in_feature.GetField(field) for field in dgo_fields if field != 'HUC10'}
...
def build_output(output_gpkg: str, huc10_path: str, hucs: list, huc_attributes: str, reset: bool) -> None:
    ...
    with sqlite3.connect(output_gpkg) as conn:
        ...
        conn.execute('CREATE INDEX vbet_igos_huc10_idx ON vbet_igos (huc10, levelPathI, seg_distance)')
