...
from lib import api
from lib.monitor import print_job, possible_states
from lib.cloudwatch import download_job_logs
...
def get_params(job_obj):
    ...
    if program is None or len(program) == 0:
        raise Exception('Could not find a valid PROGRAM environment variable in your job.')
    # to keep the env var short and sweet dump anything that's not this program
    rsconfig['default'] = program
    rsconfig['programs'] = {k: v for k, v in rsconfig['programs'].items() if k == program}

    # Checking for sanity here and common sense errors if needed
    if len(rsconfig['programs']) == 0:
        raise Exception('could not find program with key "{}" in your .riverscapes file.'.format(program))
    elif len(rsconfig['programs']) > 1:
        raise Exception('Found {} keys for program with key "{}" in your .riverscapes file. This can cause problems.'.format(len(rsconfig['programs']), program))
    ...
    if 'TAGS' in new_job_env and new_job_env['TAGS'] == 'TEST_ALL':
        new_job_env['TAGS'] += ',{}'.format(datetime.now().strftime("%b%d").upper())

    new_job_env['RS_CONFIG'] = json.dumps(rsconfig)
    new_job_env['NO_UI'] = 'gnarly'  # NO_UI keeps the progress bars at bay value doesn't matter
    ...
    def create_task(huc, resources=None):
        ret_obj = {
            ...
            "env": json.dumps({'HUC': huc})  # Task environment variables are unique to each task
        }
    ...
    if job_resources is not None:
        if "cpu" not in job_resources or "memory" not in job_resources or "disk" not in job_resources:
            raise Exception('If you use "resources" then all of "cpu", "memory" and "disk" are required')
...
def main(job_json_dir, api_url, username, password) -> bool:
    ...
    questions = [
        inquirer.List('job',
                      ...
                      choices=[('[{}] {} </{}>'.format(jj['status'], jj['name'], jj['path']), jj['path']) for jj in jobs_sorted] + [('Quit', 'quit')],
                      ),
    ]
    ...
    QueryMonster = api.QueryMonster(api_url, username, password)
    ...
    if os.path.isfile(outputFile):
        with open(outputFile) as f:
            try:
                ...
                log.error('error parsing: {}. Recreating it \n\n{}'.format(outputFile, e))
    ...
    if job_monitor is None:
        ...
        with open(outputFile, 'w') as outfile:
            ...
            result = QueryMonster.add_job(params)
            if result is None:
    ...
    while True:
        ...
        job_monitor = QueryMonster.get_job(job_monitor['id'])
        ...
        with open(outputFile, 'w') as outfile:
            ...
            if 'RS_CONFIG' in job_monitor['env']:
                del job_monitor['env']['RS_CONFIG']  # don't show login info in the file
        ...
        if menu_choice == 'quit':
            ...
            download_job_logs(job_monitor, monitor_logs_path, download_running=True)
            log.info('DONE!')
            time.sleep(3)
        elif menu_choice == 'task_manage':
            # Get a fresh copy to work with
            manage_tasks(QueryMonster, job_monitor['id'])

    print('Goodbye!!')
    return repeat


def manage_tasks(QueryMonster, jid):
    """[summary]
    """
    # Get a fresh version
    job_monitor = QueryMonster.get_job(jid)
    ...
    menu_choice = inquirer.list_input(
        message="Choose?",
        choices=[
            ('Restart tasks from list ({}/{} available)'.format(len(ts['restartable']), total_jobs), 'restart_tasks'),
            ('Restart FAILED tasks ({}/{} available)'.format(len(ts['failed']), total_jobs), 'restart_failed'),
            ('Restart STOPPED tasks ({}/{} available)'.format(len(ts['stopped']), total_jobs), 'restart_stopped'),
            ('Restart ALL {} Restartable tasks (Stopped, Succeeded and Failed)'.format(len(ts['restartable'])), 'restart_all'),
            ('Stop tasks from list ({}/{} available)'.format(len(ts['stoppable']), total_jobs), 'stop_tasks'),
            ('Stop ALL {} Running/Queued tasks'.format(len(ts['stoppable'])), 'stop_all'),
            ('<== Menu Back', 'back')
        ],
        default=['reload']
    )
    ...
    if menu_choice == 'restart_tasks' or menu_choice == 'stop_tasks':
        ...
        questions = [
            inquirer.Checkbox('tasks',
                              message="Which tasks to {}? <space> to select. <enter> to approve".format(op_text),
                              choices=[
                                  (colored('{} ({})'.format(t['name'], t['status']), possible_states[t['status']]), t)
        ...
        if (len(answers) == 0) or not inquirer.confirm('This will restart {} tasks:'.format(len(answers))):
            return
        return change_task_status(QueryMonster, answers, op)
    ...
    # Affected all tasks of a certain type
        ...
        if inquirer.confirm('This will {} {} {} tasks:'.format(op_text, dict_key, num_available_tasks)):
            return change_task_status(QueryMonster, ts[dict_key], op)
        return
...
def change_task_status(QueryMonster, tasks, op):
    ...
    for t in tasks:
        ...
        try:
            if op == VALID_OPS['START']:
                QueryMonster.start_task(t['id'])
            elif op == VALID_OPS['STOP']:
                QueryMonster.stop_task(t['id'])
            log.info('   -Completed {} on task: {}  ({}/{})'.format(op, t['name'], counter, len(tasks)))
        except Exception as e:
            log.error('   -Error {} on task: {}  ({}/{})'.format(op, t['name'], counter, len(tasks)))
            log.error(e)
