...
def vbet(in_line_network, in_dem, in_slope, in_hillshade, in_channel_area, project_folder, huc, flowline_type='NHD',
    ...
    with sqlite3.connect(inputs_gpkg) as conn:
        ...
        all_level_paths = list(str(lp[0]) for lp in level_paths_raw if lp[0] is not None)
        level_paths_drainage = {str(lp[0]): lp[1] for lp in level_paths_raw if lp[0] is not None}
