...
class HANDReport(RSReport):
    ...
    def __init__(self, report_path, rs_project):
        ...
        self.log = Logger('HAND Report')
        self.project_root = rs_project.project_dir
        self.report_intro()

    def report_intro(self):
        realization = self.xml_project.XMLBuilder.find('Realizations').find('HAND')
...
if __name__ == '__main__':
    ...
    parser.add_argument('projectxml', help='Path to the HAND project.rs.xml', type=str)
    ...
    report = HANDReport(args.report_path, project)
    report.write()
