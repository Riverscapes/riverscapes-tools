...
def metric_engine(huc: int, in_flowlines: Path, in_vaa_table: Path, in_counties: Path, in_segments: Path, in_points: Path,
    ...
    with sqlite3.connect(intermediates_gpkg) as conn:
        ...
        curs.execute(f"""CREATE VIEW dgo_text_metrics(fid, {text_metric_names_sql}) AS SELECT dgo.fid, o.ownership, s.us_state, c.county, e.ecoregion3, f.ecoregion4, n.name, br.bratrisk, bo.bratopp FROM dgos dgo LEFT JOIN
                     ...
                     """)
        curs.execute(f"""CREATE VIEW igo_text_metrics(fid, {text_metric_names_sql}) AS SELECT igo.fid, o.ownership, s.us_state, c.county, e.ecoregion3, f.ecoregion4, n.name, br.bratrisk, bo.bratopp FROM igos igo LEFT JOIN
