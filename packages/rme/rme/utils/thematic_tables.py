...
def create_measurement_table(gpkg_path):
    conn = sqlite3.connect(gpkg_path)
    c = conn.cursor()
    c.execute(f"SELECT machine_code, data_type FROM measurements WHERE is_active = 1")
    ...
    c.execute(f"INSERT INTO gpkg_contents (table_name, data_type) VALUES ('dgo_measurements', 'attributes')")
    conn.commit()
    conn.close()
