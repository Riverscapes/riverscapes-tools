...
def generate_gage_data(db_path, huc, minimum_gages):
    ...
    curs.execute(f"SELECT da, min_discharge, peak_discharge FROM sites LEFT JOIN discharges on sites.site_no = discharges.site_no WHERE huc = '{huc}'")
    ...
    if len(selection) >= minimum_gages:
        ...
        curs.execute(f"SELECT da, min_discharge, peak_discharge FROM sites LEFT JOIN discharges on sites.site_no = discharges.site_no WHERE SUBSTR(huc, 0, 7) = '{huc[:6]}'")
        selection = curs.fetchall()
        if len(selection) >= minimum_gages:
            for gage in selection:
                if None not in gage:
            ...
            curs.execute(f"SELECT da, min_discharge, peak_discharge FROM sites LEFT JOIN discharges on sites.site_no = discharges.site_no WHERE SUBSTR(huc, 0, 5) = '{huc[:4]}'")
            selection = curs.fetchall()
            if len(selection) >= minimum_gages:
                for gage in selection:
                    if None not in gage:
