...
Inputs in: `/home/narlorin/udata/blm/pasture_polygons_2026-03-24/inputs.json`
...
* save output...? - I've copied it below. We'll need to write it somewhere. The log is not designed for long-term, and indeed currently  is overwritten whenever run the process (and for expediency when we run into issues we don't always restart the code at step 1 each time, we comment out that part and go to step 2 or whatever)

```text
[INFO] [Vector Prep] Reading input dataset with GeoPandas: /home/narlorin/udata/blm/pasture_polygons_2026-03-24/BLM_Natl_Grazing_Pasture_Polygons_654070438877449879.gpkg
[DEBUG] [Vector Prep] No layer name specified, geopandas will choose default/first layer
[INFO] [Vector Prep] Loaded 53454 features. CRS: EPSG:4269
[INFO] [Vector Prep] Geometry type of input: {'MultiPolygon': 53454}
...
[INFO] [Vector Prep] Input features: 53454
...
[INFO] [Vector Prep] Remaining features to write: 53454
[INFO] [Output GDF] Reprojecting to EPSG 4326 for output
[INFO] [Output GDF] Writing cleaned layer to /home/narlorin/ucode/riverscapes-tools/dist/usgov_sources/blm-natl-grazing-pasture-polygons/2026-03-24/blm-natl-grazing-pasture-polygons.gpkg (driver=GPKG)...
[INFO] [Output GDF] Write complete.
[INFO] [Vector Prep Orchestrate] Prepped dataframe with shape (53454, 13) and outputed to /home/narlorin/ucode/riverscapes-tools/dist/usgov_sources/blm-natl-grazing-pasture-polygons/2026-03-24/blm-natl-grazing-pasture-polygons.gpkg
```
...
This step I might do in datagrip - SQL is most convenient. but this is in WSL filesystem... 

The following are all unique:
* globalid
* Original_GlobalID
* fid

if we consider this a "MANY_ROWS_PER_ENTITY" type dataset, with `st_allot_past` as the entity id field, then does that mean we need a name that matches this?

Some like NMF02000 have PAST_NO = 99, multiple pasture names (ALIMITO_01 to ALIMITO_12).

Let's take the "min" ADMIN_ST + ALLOT_NAME + PAST_NAME
and add a "multi" flag (0/1) for each ST_ALLOT_PAST 

### build Metadata (layer_definitions)

* fn build_layer_defs in orchestrate
...
`s3://riverscapes-athena/usgov_sources/blm-natl-grazing-pasture-polygons/2026-03-24/`

### Build Athena table

* scripted `build_athena_ddl`
* ran the script and it works. 
