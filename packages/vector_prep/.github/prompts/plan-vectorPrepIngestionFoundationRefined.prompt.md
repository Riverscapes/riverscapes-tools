...
5. Add a Phase 1 parquet export script that guarantees EPSG:4326 before write and emits parquet under `dist/parquet/{dataset_id}/{run_id}/...`; include deterministic column ordering and null-safe type handling aligned with Athena ingestion expectations. *depends on 2*
...
10. Add source download mode, S3 raw/prepared/analytics upload, and immutable versioned prefixes keyed by ingestion date/run id.
11. Implement Athena naming convention: in `rs_raw`, create snapshot-specific physical tables per layer (`{layer}_snapshot{snapshot_id}` pattern); in `rs_rpt`, expose latest-only stable objects for reporting.
...
- `NEW: /home/narlorin/ucode/riverscapes-tools/packages/vector_prep/vector_prep/export_to_parquet.py` — Phase 1 parquet export utility.
...
- Confirmed: create run manifests now; formal standalone JSON Schema file is deferred unless needed immediately for external validation.
...
- Confirmed: Athena pattern is snapshot-specific physical tables in `rs_raw`, with latest-only stable reporting objects in `rs_rpt`.
...
1. Evaluate whether `rs_raw` should use many snapshot-suffixed tables or one partitioned table per layer with snapshot_id partition; keep reporting contract in `rs_rpt` unchanged either way.
