...
def reach_geometry_NEW(flow_lines: Path, dem_path: Path, buffer_distance: float, epsg: str):
    ...
    # TODO: check with Matt the best way to reproject vectors onto raster projection
    ...
    with get_shp_or_gpkg(flow_lines) as lyr:
        ...
        transform_to_raster = lyr.get_transform(lyr.spatial_ref, out_srs)
        transform_to_output = lyr.get_transform_from_epsg(epsg)

        for feature, _counter, _progbar in lyr.iterate_features("Processing reaches"):
            reach_id = feature.GetField('ReachID')
            ...
            line_start_polygons[reach_id] = VectorBase.ogr2shapely(geom, transform_to_raster).coords[0].buffer(vector_buffer)
            line_end_polygons[reach_id] = VectorBase.ogr2shapely(geom, transform_to_raster).coords[0].buffer(vector_buffer)

    # Retrieve the mean elevation of each point
    ...
    for reach_id, data in reaches.items():
        if reach_id in line_start_elevations and reach_id in line_end_elevations:
            data['iGeo_ElMax'] = max(line_start_elevations['Max'], line_end_elevations['Max']),
            data['iGeo_ElMin'] = min(line_start_elevations['Min'], line_end_elevations['Min'])
            if line_start_elevations['Mean'] != line_end_elevations['Mean']:
                data['iGeo_Slope'] = abs(line_start_elevations['Mean'] - line_end_elevations['Mean']) / data['iGeo_Len']
        else:
            log.warning('{:,} features skipped because one or both ends of polyline not on DEM raster'.format(dem_path))

    write_attributes(os.path.dirname(flow_lines), reaches, ['iGeo_Len', 'iGeo_ElMax', 'iGeo_ElMin', 'iGeo_Slope'])
