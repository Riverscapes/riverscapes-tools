...
def riverscapes_dam_counts(gpkg_path: str, windows: dict):
    ...
    dgo = os.path.join(gpkg_path, 'beaver_activity_dgos')
    igo = os.path.join(gpkg_path, 'beaver_activity_igos')
        ...
        for igo_ftr, *_ in igo_lyr.iterate_features('Getting info from DGOs'):
            ...
            for dgo_ftr, *_ in dgo_lyr.iterate_features(attribute_filter=sql):
                if dgo_ftr.GetField('centerline_length') is None or dgo_ftr.GetField('centerline_length') == 0:
                    continue
                dam_ct += dgo_ftr.GetField(f'dam_ct')
    ...
    with GeopackageLayer(igo, write=True) as igo_lyr:
        igo_lyr.ogr_layer.CreateField(ogr.FieldDefn(f'dam_ct', ogr.OFTInteger))
        igo_lyr.ogr_layer.CreateField(ogr.FieldDefn(f'dam_density', ogr.OFTReal))
    ...
    for fid, dam_ct in out_data.items():
        ct += 1
        progbar.update(ct)
        conn.ExecuteSQL(f'UPDATE beaver_activity_igos SET dam_ct = {dam_ct[0]}, dam_density = {dam_ct[0] / (dam_ct[1] / 1000)} WHERE fid = {fid}')

    conn.ExecuteSQL('UPDATE beaver_activity_igos SET dam_ct = 0, dam_density = 0 WHERE dam_ct IS NULL')
