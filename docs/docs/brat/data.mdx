...
| Data Type | Integer |
| Units | NA |
| Description | Identifies the land ownership for the property underlying the reach. This is calculated by intersecting the feature polyline with the land ownership layer and determining the agency that possesses the greatest length of reach. The ID is a lookup on the `Agencies` database table. |
